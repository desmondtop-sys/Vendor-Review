...
from defs import Vendor_Page_Tabs
...
def render_vendors_page() -> None:
    ...
    vendor_title_col, search_bar_col, new_vendor_button_col = st.columns([0.2, 0.6, 0.2])
    with vendor_title_col:
        st.title("🏢 Vendors")
    
    with search_bar_col:
        search_query = st.text_input("🔍 Search vendors", placeholder="Search by name", key="vendor_search")

    st.divider()

    with new_vendor_button_col:
        st.markdown("<div style='font-size: 18px;'>&nbsp;</div>", unsafe_allow_html=True)   # Spacer to vertically align the button with the title
    ...
    if st.session_state.get("current_tab") == Vendor_Page_Tabs.VENDOR_LIST.value:
        render_vendor_list_tab()

    elif st.session_state.get("current_tab") == Vendor_Page_Tabs.HEAT_MAP.value:
...
def render_vendor_page_tabs() -> None:
    ...
    with tab1:
        type1 = "primary" if st.session_state.current_tab == Vendor_Page_Tabs.VENDOR_LIST.value else "secondary"
    ...
    with tab2:
        type2 = "primary" if st.session_state.current_tab == Vendor_Page_Tabs.HEAT_MAP.value else "secondary"

        if st.button("Heat Map", type=type2, width='stretch'):
            st.session_state.current_tab = Vendor_Page_Tabs.HEAT_MAP.value
            st.rerun()
